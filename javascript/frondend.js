...
function showCategory(categoryId, event = null) {
  ...
  if (event) {
    ...
    var initialSelectedButton = document.querySelector(`.category[data-category="${categoryId}"]`);
  ...
  function activateWebcam() {
    navigator.mediaDevices.getUserMedia({ video: true })
      .then(stream => {
        ...
        webcam.play().catch(error => console.error("Error playing video stream:", error));
...
function updateDisplayState(buttonType) {
  ...
  if (buttonType === 'preview') {
    elementsToShow.forEach(element => {
        if (element !== logo && element !== chatContainer && element !== languagesFlags) {
  ...
  conversationButton.classList.toggle('inactive', buttonType !== 'conversation');
...
function toggleMenu(menuId) {
  ...
  const otherSubMenus = document.querySelectorAll('.sub-menu:not(#' + menuId + 'SubMenu)');
...
function openFirstButton(menuId) {
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    const subMenuButtons = document.querySelectorAll('.sub-menu button.transparent-button ');
  ...
  function toggleSubMenu(subMenu) {
    ...
    // Hide other sub-menus
    const otherSubMenus = document.querySelectorAll('.sub-menu:not(' + subMenu.id + ')');
...
function commonDropHandler(e, commandPrefix) {
  ...
  const draggedImageAlt = htmlDoc.querySelector('img') ? htmlDoc.querySelector('img').alt : '';
...
dropZones.forEach((dropZone) => {
  ...
  document.getElementById('birthmarkButton').addEventListener('click', function() {
...
function updateVitiligoSliderValue() {
 ...
 document.getElementById('vitiligoButton').addEventListener('click', function() {
...
document.getElementById('frecklesButton').addEventListener('click', function () {
