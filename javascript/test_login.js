...
document.getElementById('login-button').addEventListener('click', async (event) => {
    ...
    const query = new URLSearchParams({
        ...
        login_url: 'https://main--phenomenal-axolotl-f5de44.netlify.app/dashboard',
        with_logout: '0'
    }).toString();

    try {
        const resp = await fetch(
            `https://login.xsolla.com/api/login?${query}`,
            {
                method: 'POST',
                headers: {
        ...
        loginErrorMessage.textContent = 'An error occurred during authentication. Please try again later.';
        loginErrorMessage.style.display = 'block'; // Show the error message element
    }
});
...
// Forgot Xsolla api
document.addEventListener('DOMContentLoaded', function() {
    document.querySelector('#forgot-password-section form').addEventListener('submit', async (event) => {
        event.preventDefault(); // Prevent the default form submission

        const emailInput = document.getElementById('forgot-password-email');
        const successMessageDiv = document.getElementById('forgot-password-success-message');
        const errorMessageDiv = document.getElementById('forgot-password-error-message');
        ...
        try {
            ...
            if (response.status === 204) {
                ...
                successMessageDiv.textContent = 'Password reset request sent successfully. Check your email for further instructions.';
            ...
            errorMessageDiv.textContent = 'An error occurred during the password reset request. Please try again later.';
            errorMessageDiv.classList.remove('hidden');
        }
    });
});
