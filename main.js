...
document.getElementById('exportAvatarButton').addEventListener('click', exportAvatar);
document.getElementById('exportRawBaseMeshButton').addEventListener('click', exportRawBaseMesh);
document.getElementById('exportAvatarHeadButton').addEventListener('click', exportAvatarHead);
document.getElementById('randomizeButton').addEventListener('click', generateAvatar);
...
async function fetchBlendshapeData(user_info_id) {
  try {
    const response = await fetch(`/.netlify/functions/getBlendshape?user_info_id=${user_info_id}`);
    if (!response.ok) {
...
async function fetchAvatarData(user_info_id) {
  try {
    const response = await fetch(`/.netlify/functions/getUserAvatars?user_info_id=${user_info_id}`);
    const data = await response.json();
...
async function displayAvatarNames(avatars) {
  ...
  for (const avatar of avatars) {
    ...
    avatarImg.src = avatar.Avatar_Image ? `data:image/jpeg;base64,${avatar.Avatar_Image}` : 'src/Default_Avatar_Icon.png';
  ...
  avatarDiv.addEventListener('click', async () => {
    ...
    if (selectedAvatar) {
      ...
      console.log(`JSON Data for Avatar ID ${selectedAvatarId}:`, avatarJsonData);

      handleSendCommands(avatarJsonData);

      const previouslySelectedAvatar = document.querySelector('.avatar.selected');
      ...
      if (avatarJsonData && avatarJsonData.Personas) {
        console.log(`Personas Information for Avatar ID ${selectedAvatarId}:`, avatarJsonData.Personas);
        
        // Display the Personas information in the input field
        document.getElementById('personaInput').value = avatarJsonData.Personas;

      } else {
        console.log(`No Personas information found for Avatar ID ${selectedAvatarId}.`);
      }
      ...
      document.getElementById('avatarName').textContent = selectedAvatar.Avatar_Name;
...
async function deleteAvatar(selectedAvatarId) {
  try {
    const response = await fetch(`/.netlify/functions/deleteAvatar/${selectedAvatarId}`, {
      method: 'DELETE',
      headers: {
        'Content-Type': 'application/json',
      },
...
async function addAvatarToDatabase(username, user_info_id) {
  try {
    ...
    const data = await response.json();
    console.log('Avatar name added successfully:', username, 'with ID:', data.saveavatar);
...
async function uploadLogo(base64Data) {
  ...
  try {
      const response = await fetch(`/.netlify/functions/uploadLogo?user_info_id=${user_info_id}`, {
          method: 'POST',
          headers: {
              'Content-Type': 'application/json'
          },
...
function loadLogo() {
  ...
  async function fetchTierName(user_info_id) {
    try {
        const response = await fetch(`/.netlify/functions/user-tier?user_info_id=${user_info_id}`);
        const data = await response.json();
        if (response.ok) {
            document.getElementById('tier').textContent = `Reblium: ${data.tier_name}`;
        } else {
            console.error('Failed to fetch tier name:', data.error);
        }
    } catch (error) {
        console.error('Error fetching tier name:', error);
        document.getElementById('tier').textContent = 'Error fetching tier information.';
    }
}
...
document.getElementById('save-exit').addEventListener('click', () => {
  ...
  async function initPage() {
    try {
      ...
      const checkUserExistsResponse = await fetch(`/.netlify/functions/checkUserExists?user_id=${userId}`);
